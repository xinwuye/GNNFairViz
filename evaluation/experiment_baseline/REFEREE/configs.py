...
def arg_parse():
    ...
    parser.set_defaults(datadir='data', logdir='log', dataset='german', opt='adam', opt_scheduler='none',
                        cuda='0', lr=0.001, clip=2.0, batch_size=20, num_epochs=1000,
                        ...
                        explainer_backbone='PGExplainer',
                        remove=False)
...
def arg_parse_explain():
    ...
    parser.set_defaults(logdir="log", ckptdir="ckpt", dataset="german", opt="adam", opt_scheduler="none", cuda="0",
                        ...
                        dropout=0.0, method="GAT", explainer_backbone='PGExplainer', debug_mode=False, explain_all=False, baseline=False,
                        threshold=500, size_weight=5e-5)
    return parser.parse_args()



def arg_parse_explain_notebook(args=None):
    parser = argparse.ArgumentParser()
    io_parser = parser.add_mutually_exclusive_group(required=False)
    io_parser.add_argument("--dataset", dest="dataset", help="Input dataset.")
    benchmark_parser = io_parser.add_argument_group()
    benchmark_parser.add_argument("--bmname", dest="bmname", help="Name of the benchmark dataset")
    io_parser.add_argument("--pkl", dest="pkl_fname", help="Name of the pkl data file")
    # Assuming util.parse_optimizer(parser) is defined elsewhere in your code.
    util.parse_optimizer(parser)
    parser.add_argument("--hidden-dim", dest="hidden_dim", type=int, help="Hidden dimension")
    parser.add_argument("--output-dim", dest="output_dim", type=int, help="Output dimension")
    parser.add_argument("--num-gc-layers", dest="num_gc_layers", type=int,
    ...
    if args:
        ...
        # If no arguments are provided, return defaults
        return parser.parse_args([])