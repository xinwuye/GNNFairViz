...
def train_node_classifier(G, labels, model, args):
    ...
    ypred = None
    # print('in train_node_classifier')
...
def task_real_data(args):
    ...
    if args.gpu:
        model = model.cuda()
    # print('before train_node_classifier')
    # print_gpu_memory_stats()
    return train_node_classifier(G, labels, model, args)

def preprocess_input_graph(G, labels):
    return {'labels': labels[None, ...], 
            'feat': np.array(list(nx.get_node_attributes(G, 'feat').values()))[None, ...],
