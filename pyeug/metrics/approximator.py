...
def wasserstein_cost_loss(output, sens):
    # unique_sens = torch.unique(sens)
    unique_sens = np.unique(sens)
    num_classes = len(unique_sens)
    pairwise_distances = []
    
    # Compute pairwise Wasserstein distances for all unique group pairs
    for (i, j) in combinations(unique_sens, 2):
        group_i_diff = output[sens == i]
        group_j_diff = output[sens == j]
        pairwise_distances.append(wasserstein(group_i_diff, group_j_diff, 'cuda'))
    
    # Sum the distances and normalize
    sum_pairwise_distances = torch.sum(torch.stack(pairwise_distances))
    normalized_wasserstein_distance = (2.0 / (num_classes * (num_classes - 1))) * sum_pairwise_distances
    
    return normalized_wasserstein_distance

def grad_z_graph_faircost(g, features, idx, labels, sens, model, gpu=-1):
    model.eval()
    # initialize
    if gpu >= 0:
        features, labels = features.cuda(), labels.cuda()
    if gpu < 0:
        features, labels, model = features.cpu(), labels.cpu(), model.cpu()
    ...
    if True:
        ...
        loss2 = wasserstein_cost_loss(output[idx][labels[idx.cpu()] == 1], sens[idx][(labels[idx] == 1).cpu()])
...
def s_test_graph_cost(g, features, idx_train, idx_test, labels, sens, model, gpu=-1, damp=0.03, scale=60,
    ...
    v = grad_z_graph_faircost(g, features, idx_test, labels, sens, model, gpu)
    ...
    for i in range(recursion_depth):
        ...
        # output = model(features, adj)
        output = model(g, features)
        out, label = output[random_train_idx], labels[random_train_idx]
...
def grad_z_graph(g, features, idx, labels, model, gpu=-1):
    ...
    for i in range(len(idx)):
        # output = model(features, adj)
        output = model(g, features)
        out, label = output[idx[i]], labels[idx[i]]
        # loss = F.binary_cross_entropy_with_logits(out, label)
        # print(label)
        loss = nn.CrossEntropyLoss(ignore_index=-1)(out, label)
        params = [p for p in model.parameters() if p.requires_grad]
