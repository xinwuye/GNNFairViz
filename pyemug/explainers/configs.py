import argparse
# import util
from . import util_explainers as util
...
def arg_parse():
    ...
    parser.add_argument('--debias_ratio', dest='debias_ratio', type=int,
                        help='percentage')
    parser.set_defaults(datadir='data', logdir='log', dataset='german', opt='adam', opt_scheduler='none',
                        ...
                        output_dim=20, num_classes=2, num_gc_layers=3, dropout=0.0, weight_decay=0.005, method='gat', debias_ratio=20,
...
def arg_parse_explain():
    ...
    parser.add_argument("--gpu", dest="gpu", action="store_const", const=True, 
                        # default=True,
                        default=False,
                        help="whether to use GPU.", )
    ...
    parser.set_defaults(logdir="log", ckptdir="ckpt", dataset="german", opt="adam", opt_scheduler="none", cuda="0",
                        lr=0.5, clip=2.0, batch_size=20, num_epochs=20, hidden_dim=20, output_dim=20, num_gc_layers=3,
                        dropout=0.0, method="GAT", explainer_backbone='GNNExplainer', debug_mode=False, explain_all=False, group_sample=0,
                        threshold=5000, size_weight=5e-5)
