...
class IndividualExplainer:
    ...
    def explain(self, sens, id): # explain a single node
        seed = 100
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)

        node_idx_new, sub_adj, sub_feat, neighbors = self.extract_neighborhood(id)
        ...
        # for epoch in range(self.args.num_epochs):
            ...
            loss = explainer_unfair.loss(node_idx_new, self.tensor_pred.clone())
        ...
        result=pd.DataFrame([{'dataset':self.args.dataset, 
                              ...
                              'Delta B (Promoted)': '{:.4f}'.format(-(wass_dis - (-explainer_unfair.loss(node_idx_new, self.tensor_pred.clone()))) / wass_dis * 1e4), # positive value with larger absolute value is better
                              'original wass_dis': '{}'.format(wass_dis),
                              'new_wass_dis': '{}'.format(-explainer_unfair.loss(node_idx_new, self.tensor_pred.clone())),
                              }])
        ...
        #     f.write(str(float(-(wass_dis - (-explainer_unfair.loss(node_idx_new, self.tensor_pred.clone()))) / wass_dis * 1e4)) + ',')

        return masked_adj_unfair, neighbors

    def extract_neighborhood(self, node_idx):
        # neighbors_adj_row = self.neighborhoods[0][node_idx, :]
        neighbors_adj_row = self.neighborhoods[node_idx, :] # the i-th row indicates the neighbors of the i-th node
        node_idx_new = sum(neighbors_adj_row[:node_idx])
        neighbors = np.nonzero(np.array(neighbors_adj_row))[0] # the indices of neighbors. The np.nonzero() function is a NumPy function that returns the indices of the non-zero elements in an array. It returns a tuple of arrays, one for each dimension of the input array, containing the indices of the non-zero elements
        # sub_adj = self.adj[0][neighbors][:, neighbors]
        sub_adj = self.adj[neighbors][:, neighbors] # adj of computational graph of this node
        # sub_feat = self.feat[0, neighbors]
        sub_feat = self.feat.flatten(start_dim=0, end_dim=-2)[neighbors] # feature of computational graph of this node
        return node_idx_new, sub_adj, sub_feat, neighbors # node_idx_new is the index of the node in the subgraph(i.e. computational graph), sub_adj is the adjacency matrix of the subgraph, sub_feat is the feature matrix of the subgraph, sub_label is the label of the subgraph, neighbors is the indices of the nodes in the subgraph in the original graph
...
class ExplainModule(nn.Module):
    ...
    def loss(self, id, tensor_pred):
        ...
        current_pred = self.model(self.x, new_adj)[0].flatten(start_dim=0, end_dim=-2)[id]
        tensor_pred[id] = current_pred
        wass_dis = 0
